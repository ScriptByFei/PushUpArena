      ...
      <Card className="relative bg-gradient-to-br from-brand-700/40 to-ink-800/70">
        ...
        <div className="relative mt-4">
          {xpBurst && (
            <span
              key={xpBurst.id}
              className="pointer-events-none absolute -top-5 right-0 animate-xp-float text-sm font-bold text-brand-200"
            >
              +{xpBurst.amount} XP
            </span>
          )}
          ...
          <div className="mt-1.5 flex items-center justify-between text-xs">
            <span className="text-slate-400">🎁 Nächste Belohnung bei {nextLevelXp} XP</span>
            <span className="text-slate-500">noch {progress.xpToNext} XP</span>
          </div>
        </div>
